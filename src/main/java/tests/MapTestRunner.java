...
public class MapTestRunner {
    ...
    for (final int mapSize : MAP_SIZES) {
      ...
      for (final Class<?> testClass : tests) {
        Options opt = new OptionsBuilder()
          ...
          //.result("target/" + DateTimeFormatter.ofPattern("dd-HH-mm").format(LocalDateTime.now()) + "-" + testSetName + ".json")
  ...
  public void setup() throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
    final ITestSet testSet = (ITestSet) Class.forName(m_className).getDeclaredConstructor().newInstance();
    switch (m_testType) {
      case "get":
